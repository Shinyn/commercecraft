...
export async function GET(
  ...
  if(status=="all"||status=="undefined"){
    ...
    try {
      ...
      return new NextResponse('Ooops, something went wrong when getting the order', { status: 500 });
    }
  }
}
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    return new NextResponse('Something went wrong when trying to update the order', { status: 500 });
  }
}
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    return new NextResponse('Something went wrong when trying to delete the order', { status: 500 });
  }
}
