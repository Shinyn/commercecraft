...
export async function PUT(
    ...
    try {
        ...
        if (active) {//Remove activation from any previous billboards if needed
            const updated = await prismadb.billboard.updateMany({
                where: { active: 1 },
                data: { active: 0 },
            })
                .then(async function (response) {//update the billboard
        ...
        console.log("api/billboards/POST", error)
        return new NextResponse("Something went wrong when trying to update your billboard", { status: 500 })
    }
}
